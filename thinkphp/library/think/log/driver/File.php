...
class File
{
    ...
    public function save(array $log = [], $append = false)
    {
        ...
        !is_dir($path) && umask(0) && mkdir($path, 0777, true);
...
//            }
    ...
    {
        if (is_array($info) && (isset($info['error']) || isset($info['ERROR']))) {
            $type = 'ERROR';
        } else {
            $type = 'INFO';
        }
        ...
        if (true) {
            $message = '';
            $message .= date('Y-m-d H:i:s') . str_replace('0.', ',', bcmod(microtime(true), 1, 4)) . ' ';
            $message .= '[cli] ';
            $message .= $type . ' ';
    ...
    protected function parseLog($info)
    {
        ...
        if (true) {
            ...
            $data = '';
            $data .= date('Y-m-d H:i:s') . str_replace('0.', ',', bcmod(microtime(true), 1, 4)) . ' ';
            $data .= '[web] ';
            $data .= $type . ' ';
            $data .= json_encode($info, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) . "\r\n";
            return $data;
        }
